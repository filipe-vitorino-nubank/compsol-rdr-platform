      ...
      <div className="profile-drawer">
        <div className="profile-drawer-header">
          <div>
            <h3>{t("nav.profile")}</h3>
            {lastSync && (
              <span className="profile-last-sync">{lastSync}</span>
            )}
          </div>
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            ...
            >
              ...
              {syncing ? t("profile.syncing") : t("profile.syncBtn")}
            </button>
        ...
        <div className="profile-self-card">
          <div className="profile-self-avatar">
            {googleUser?.picture ? (
              <img src={googleUser.picture} alt={googleUser.name ?? ""} referrerPolicy="no-referrer" />
            ) : (
              <span className="profile-self-initials">
                {(googleUser?.name ?? googleUser?.email ?? "?").charAt(0).toUpperCase()}
              </span>
            )}
          </div>
          <div>
            <span className="profile-self-name">{googleUser?.name ?? "—"}</span>
          </div>
        </div>

        <div className="profile-section-label">{t("profile.teamLabel")}</div>
